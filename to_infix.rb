# define infix_args, from, is_flipped, op to make this work
module ToInfix
  def to_infix(is_rhs=false)
    return "()" if from && from.token && from.token.str == :EOL

    op_name = (op.sym||((is_flipped ? "" : " ")+op.name+" ")).to_s
    op_name = "‿" if op.sym == " " # todo and type inferred already
    op_str = (is_flipped ? "@" : "") + op_name
