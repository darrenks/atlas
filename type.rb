...
Type = Struct.new(:rank,:base) # base is :num, :char, :a, or [...] of type
...
class Type
  def inspect
    ...
    base_s = if Array === base
      "[" + Type.new(0,base[0]).inspect + "]"
    else
      base.to_s.size>1 ? base.to_s.capitalize : base.to_s
    end
    "<"*rank + base_s + ">"*rank
  end
  def -(rhs)
  ...
  def unbox_all
    if Array === base
      Type.new(rank+1,base[0]).unbox_all
    else
      self
    end
  end
